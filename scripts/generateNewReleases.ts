...
type ReleaseFormatted = Omit<Release, "notes" | "changelog_bitcoinonly" | "fingerprint_bitcoinonly" | "url_bitcoinonly">;

const FW_DATA = "../firmware";

const transformReleases = async (deviceModel: string) => {
  const dirPath = path.join(__dirname, `${FW_DATA}/${deviceModel}/releases.json`);
  ...
  try {
    ...
    const universalOutputDirPath = path.join(__dirname, "..", "suite", "firmware", deviceModel, "universal");
    await fs.promises.mkdir(universalOutputDirPath, { recursive: true });
    const bitcoinonlyOutputDirPath = path.join(__dirname, "..", "suite", "firmware", deviceModel, "bitcoinonly");
    await fs.promises.mkdir(bitcoinonlyOutputDirPath, { recursive: true });

    for (const release of releases) {
      const universalFileName = `${deviceModel}-${release.version.join(".")}-universal.json`;
      const universalBinName = `${deviceModel}-${release.version.join(".")}-universal.bin`;
      const universalOutputPath = path.join(universalOutputDirPath, universalFileName);
      ...
      await writeFile(universalOutputPath, universalJsonString, { encoding: "utf8" });
      console.log(`Generated for universal file: ${universalOutputPath}`);

      if (release.fingerprint_bitcoinonly) {
        const bitcoinonlylFileName = `${deviceModel}-${release.version.join(".")}-bitcoinonly.json`;
        const bitcoinonlylBinName = `${deviceModel}-${release.version.join(".")}-bitcoinonly.bin`;
        const bitcoinonlyOutputPath = path.join(bitcoinonlyOutputDirPath, bitcoinonlylFileName);
        ...
        await writeFile(bitcoinonlyOutputPath, bitcoinonlyJsonString, { encoding: "utf8" });
